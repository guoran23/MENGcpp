...
void ParticleExtCls::particle_ext_cls_dxvpardt123EM2d1f(
  ...
  this->particle_ext_cls_dxvpardt123EMgeneral(
      ...
      // Optional keyword arguments from Fortran are passed explicitly in C++
      phik, apark, ntor1d);
}
...
void ParticleExtCls::particle_ext_cls_dxvpardt123EMgeneral(
  ...
  if (this->batch_g2p) {
    ...
    { // 2D1F
      ...
      // Phi梯度
      fd.field_cls_g2p2d1f_grad(equ, phik_c, ntor1d, partrad0, parttheta0,
                                partphitor0, pt1ddPdrad, pt1ddPdthe, pt1ddPdphi,
                                ngyro, rho1arr);

      // A梯度
      fd.field_cls_g2p2d1f_grad(equ, apark_c, ntor1d, partrad0, parttheta0,
      ...
      //   if (this->imixvar == 0)
      {
        ...
        fd.field_cls_g2p2d1f_general(
            equ, apark_c, ntor1d, partrad0, parttheta0, partphitor0, pt1dAorAh,
  ...
  for (int fic = 0; fic < nptot; ++fic) {
    ...
    double ptA;
    double ptdAsdrad, ptdAsdthe, ptdAsdphi;
    double ptEB_dphidt;
    ...
    fd.field_cls_g2p2d1f_grad(equ, apark_c, ntor1d, ptrad, pttheta, ptphitor,
    ...
    if (ischeme_motion == 1) {
      ...
      // Bstar + As修正项
      bstarAs_rad_ct = bstar_rad_ct +
                       JB_inv * (ptdAsdphi * ptBthe_co - ptdAsdthe * ptBphi_co);
      bstarAs_the_ct = bstar_the_ct +
                       JB_inv * (ptdAsdrad * ptBphi_co - ptdAsdphi * ptBrad_co);
      bstarAs_phi_ct = bstar_phi_ct +
                       JB_inv * (ptdAsdthe * ptBrad_co - ptdAsdrad * ptBthe_co);
    ...
    //// === 调用场（field）插值 ===
    fd.field_cls_g2p2d1f_general(equ, apark_c, ntor1d, ptrad, pttheta, ptphitor,
    ...
    if (this->batch_g2p) {
      ...
      fd.field_cls_g2p2d1f_grad(equ, phik_c, ntor1d, ptrad, pttheta, ptphitor,
    ...
    ptdvpardt1 = ptdvpardt1 + ptCvpar * (ptBphi_ct * ptdBdthe * ptdAsdrad -
                                         ptBphi_ct * ptdBdrad * ptdAsdthe +
                                         ptBthe_ct * ptdBdrad * ptdAsdphi);
