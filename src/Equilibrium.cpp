#include "Equilibrium.h"
#include <iostream>

Equilibrium::Equilibrium(double p, double t) : pressure(p), temperature(t) {}

double Equilibrium::getPressure() const {
    return pressure;
}

double Equilibrium::getTemperature() const {
    return temperature;
}

void Equilibrium::adjustEquilibrium(double dPressure, double dTemperature) {
    pressure += dPressure;
    temperature += dTemperature;
}

void Equilibrium::displayEquilibrium() const {
    std::cout << "Pressure: " << pressure << " Pa, Temperature: " << temperature << " K\n";
}

void Equilibrium::equil_cls_showinfo(int rank)
{
    if (rank == 0)
    {
        // Formatting strings (similar to Fortran sform_r and sform_i)
        std::cout << std::setfill(' ');

        // Integer format (e.g., "iequmodel=" followed by the value)
        std::cout << std::setw(14) << "iequmodel=" << std::setw(10) << iequmodel
                  << '\n';

        // Real numbers with specific formatting (scientific notation)
        std::cout << std::setw(14) << "c1adhoc=" << std::scientific
                  << std::setprecision(5) << std::setw(12) << c1adhoc
                  << ", c2adhoc=" << c2adhoc << ", rmaxis_adhoc=" << rmaxis_adhoc
                  << ", Bmaxis_adhoc=" << Bmaxis_adhoc << '\n';

        // Integer format for 'isource_ref'
        std::cout << std::setw(14) << "isource_ref=" << std::setw(10) << isource_ref
                  << '\n';

        // Real numbers with specific formatting for the other parameters
        std::cout << std::setw(14) << "rhoN=" << rhoN << ", betaN=" << betaN
                  << ", nref=" << nref << ", Tref=" << Tref << ", Bref=" << Bref
                  << '\n';

        // Integer format for 'isetpsi'
        std::cout << std::setw(14) << "isetpsi=" << std::setw(10) << isetpsi
                  << '\n';

        // Real number for 'fBphi'
        std::cout << std::setw(14) << "fBphi=" << fBphi << '\n';
    }
}
double Equilibrium::getradRZ(double RR, double ZZ)
{
    double var = 0.0;
    int idx, idy, iflag = 0;

    if (iequmodel == 1)
    {
        idx = 0;
        idy = 0;
        // radrz_bsp->evaluate(RR, ZZ, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getradRZ: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        var = std::sqrt((RR - rmaxis) * (RR - rmaxis) +
                        (ZZ - zmaxis) * (ZZ - zmaxis));
    }

    return var;
}

double Equilibrium::gettheRZ(double RR, double ZZ)
{
    double var = 0.0;
    double rormaj = 0.0;
    const double twopi = 2 * M_PI; // Define twopi as 2 * pi

    if (iequmodel == 1)
    {
        // For model 1, the logic is simpler
        var = std::atan2(ZZ - zmaxis, RR - rmaxis);
        var = std::fmod(var, twopi); // Use fmod to calculate the modulus
    }
    else if (iequmodel == 2)
    {
        // For model 2, more complex calculation
        rormaj = getradRZ(RR, ZZ) / rmaxis;
        var = std::atan2(ZZ - zmaxis, RR - rmaxis);
        var = 2 * std::atan(std::sqrt((1.0 - rormaj) / (1.0 + rormaj)) *
                            std::tan(var / 2.0));
        var = std::fmod(var, twopi);

        // Check for the value of var and reset to 0 if not in the valid range
        if (var < 0.0 || var > twopi)
        {
            var = 0.0;
            std::cerr << "Warning: var reset to 0 in gettheRZ (iequmodel == 2) from "
                      << var << std::endl;
        }
    }

    return var;
}
void Equilibrium::calcdrtdRZ(double rad, double the, double &drdR, double &drdZ,
                             double &dtdR, double &dtdZ, double &jaco2)
{

    double dRdrad, dRdthe, dZdrad, dZdthe, jacoinv;

    // Get the partial derivatives of R and Z with respect to rad and theta
    dRdrad = getdRdrad(rad, the);
    dRdthe = getdRdthe(rad, the);
    dZdrad = getdZdrad(rad, the);
    dZdthe = getdZdthe(rad, the);

    // Calculate the Jacobian determinant (jaco2)
    jaco2 = dRdrad * dZdthe - dRdthe * dZdrad;

    // Calculate the inverse of the Jacobian
    jacoinv = 1.0 / jaco2;

    // Calculate the derivatives of r and theta with respect to R and Z
    drdR = dZdthe * jacoinv;
    drdZ = -dRdthe * jacoinv;
    dtdR = -dZdrad * jacoinv;
    dtdZ = dRdrad * jacoinv;
}

void Equilibrium::calcBJgij(double rad, double the0, double &RR, double &ZZ,
                            double &BB, double &dBdrad, double &dBdthe,
                            double &Bthe_ct, double &Bphi_ct, double &jaco2,
                            double &FF, double &g11, double &g12, double &g22)
{
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation to wrap the angle
    double drdR, drdZ, dtdR, dtdZ;

    // Call to another member function
    calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);

    g11 = drdR * drdR + drdZ * drdZ; // Calculate g11
    g22 = dtdR * dtdR + dtdZ * dtdZ; // Calculate g22
    g12 = drdR * dtdR + drdZ * dtdZ; // Calculate g12

    FF = getfpol_r(rad);
    RR = getR(rad, the);
    ZZ = getZ(rad, the);

    BB = getB(rad, the);
    dBdrad = getdBdrad(rad, the);
    dBdthe = getdBdthe(rad, the);

    if (iequmodel == 1)
    {
        Bthe_ct = getdpsidr(rad) / (jaco2 * RR);
        Bphi_ct = FF / (RR * RR);
    }
    else if (iequmodel == 2)
    {
        Bthe_ct = getBthe_ct(rad, the0);
        Bphi_ct = FF / (RR * RR);
        // Uncomment if you want to use getBphi_ct instead:
        // Bphi_ct = getBphi_ct(rad, the0);
    }
}

// Function to calculate h_adhoc
double Equilibrium::geth_adhoc(double rad, double the, double rmaxis)
{
    double rormaj = rad / rmaxis;
    double var = (1.0 - rormaj * rormaj) / (1.0 - rormaj * std::cos(the));
    return var;
}

// Function to calculate Bthe_ct
double Equilibrium::getBthe_ct(double rad, double the0)
{
    double var = 0.0;
    double drdR, drdZ, dtdR, dtdZ, jaco2, RR;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation

    if (iequmodel == 1)
    {
        calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);
        RR = getR(rad, the);
        var = getdpsidr(rad) / (jaco2 * RR);
    }
    else if (iequmodel == 2)
    {
        var = Bmaxis /
              (rmaxis * getqloc_rt(rad, the0) * geth_adhoc(rad, the0, rmaxis) *
               geth_adhoc(rad, the0, rmaxis));
    }

    return var;
}

// Function to calculate Bphi_ct
double Equilibrium::getBphi_ct(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation

    if (iequmodel == 1)
    {
        var = getfpol_r(rad) /
              std::pow(getR(rad, the), 2); // R^2 term in the denominator
    }
    else if (iequmodel == 2)
    {
        var = Bmaxis / (rmaxis * std::pow(geth_adhoc(rad, the0, rmaxis), 2));
    }

    return var;
}
// Function to calculate B
double Equilibrium::getB(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;
    int idx = 0, idy = 0, iflag = 0;

    if (iequmodel == 1)
    {
        // For model 1, using Brt_bsp to evaluate
        // Brt_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getB: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        var = Bmaxis * (1 - rormaj * std::cos(the0)) / (1 - rormaj * rormaj) *
              std::sqrt(std::pow((rormaj / getqloc_rt(rad, the0)), 2) /
                            (1.0 - rormaj * rormaj) +
                        1.0);
    }

    return var;
}
// Function to calculate dBdrad
double Equilibrium::getdBdrad(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0, qloc = 0.0, tmpcoeff = 0.0;
    int idx = 1, idy = 0, iflag = 0;

    if (iequmodel == 1)
    {
        // For model 1, using Brt_bsp to evaluate
        // Brt_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBdrad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        qloc = getqloc_rt(rad, the0);
        tmpcoeff = 1.0 - rormaj * rormaj;

        var = ((1.0 / rad - getdqdrloc_rt(rad, the0) / qloc +
                3.0 * rormaj / (tmpcoeff) / rmaxis) *
                   std::pow(rormaj / qloc, 2) +
               2.0 * rormaj / rmaxis) *
                  (1.0 - rormaj * std::cos(the0)) * Bmaxis / (tmpcoeff * tmpcoeff) -
              (std::pow(rormaj / qloc, 2) / tmpcoeff + 1.0) * std::cos(the0) *
                  Bmaxis / (tmpcoeff * rmaxis);

        var = var / std::sqrt(rormaj * rormaj / (qloc * qloc * tmpcoeff) + 1.0);
    }

    return var;
}
// Function to calculate dB/dthe
double Equilibrium::getdBdthe(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0, qloc = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, using Brt_bsp to evaluate
        int idx = 0, idy = 1, iflag = 0;
        // Brt_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBdrad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        qloc = getqloc_rt(rad, the0);
        var = Bmaxis * rormaj * std::sin(the0) / (1.0 - rormaj * rormaj) *
              std::sqrt(std::pow(rormaj, 2) /
                            (std::pow(qloc, 2) * (1.0 - rormaj * rormaj)) +
                        1.0);
    }

    return var;
}

// Function to calculate Brad_co
double Equilibrium::getBrad_co(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, using Brad_co_bsp to evaluate
        int idx = 0, idy = 0, iflag = 0;
        // Brad_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getBrad_co: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        var = std::pow(rormaj, 2) * Bmaxis * std::sin(the0) /
              (getqloc_rt(rad, the0) * std::pow(1 - rormaj * rormaj, 2));
    }

    return var;
}

// Function to calculate dBrad_co/drad
double Equilibrium::getdBrad_codrad(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, using Brad_co_bsp to evaluate
        int idx = 1, idy = 0, iflag = 0;
        // Brad_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBrad_codrad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        var = (-rad * getdqdrloc_rt(rad, the0) / getqloc_rt(rad, the0) +
               4.0 * std::pow(rormaj, 2) / (1.0 - rormaj * rormaj)) *
              rormaj * Bmaxis * std::sin(the0) /
              (rmaxis * getqloc_rt(rad, the0) * std::pow(1.0 - rormaj * rormaj, 2));
    }

    return var;
}
// Function to calculate dBrad_co/dthe
double Equilibrium::getdBrad_codthe(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, using Brad_co_bsp to evaluate
        int idx = 0, idy = 1, iflag = 0;
        // Brad_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBrad_codthe: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        var = std::cos(the0) * std::pow(rormaj, 2) * Bmaxis /
              (getqloc_rt(rad, the0) * std::pow(1.0 - rormaj * rormaj, 2));
    }

    return var;
}

// Function to calculate Bthe_co
double Equilibrium::getBthe_co(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, using Bthe_co_bsp to evaluate
        int idx = 0, idy = 0, iflag = 0;
        // Bthe_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getBthe_co: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, performing calculations based on formula
        rormaj = rad / rmaxis;
        var = std::pow(rad, 2) * Bmaxis /
              (rmaxis * getqloc_rt(rad, the0) * std::pow(1.0 - rormaj * rormaj, 2));
    }

    return var;
}

// Function to calculate Bphi_co
double Equilibrium::getBphi_co(double rad, double the0)
{
    double var = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, retrieve fpol_r
        var = getfpol_r(rad);
    }
    else if (iequmodel == 2)
    {
        // For model 2, apply formula using Bmaxis and rmaxis
        var = Bmaxis * rmaxis;
    }

    return var;
}

// Function to calculate dBthe_codrad
double Equilibrium::getdBthe_codrad(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, use Bthe_co_bsp to evaluate
        int idx = 1, idy = 0, iflag = 0;
        // Bthe_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBthe_co/drad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, apply formula using rormaj and Bmaxis
        rormaj = rad / rmaxis;
        var = (-rad * getdqdrloc_rt(rad, the0) / getqloc_rt(rad, the0) + 2.0 +
               2.0 * std::pow(rormaj, 2) / (1.0 - std::pow(rormaj, 2))) *
              rormaj * Bmaxis /
              (getqloc_rt(rad, the0) * std::pow(1.0 - std::pow(rormaj, 2), 2));
    }

    return var;
}

// Function to calculate dBthe_codthe
double Equilibrium::getdBthe_codthe(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = 0.0;

    if (iequmodel == 1)
    {
        // For model 1, use Bthe_co_bsp to evaluate
        int idx = 0, idy = 1, iflag = 0;
        // Bthe_co_bsp.evaluate(rad, the, idx, idy, var, iflag);

        // Error handling
        if (iflag != 0)
        {
            std::cerr << "****Error in getdBthe_co/dthe: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, return zero (as per the original code)
        rormaj = rad / rmaxis;
        var = 0.0;
    }

    return var;
}
// Function to calculate Bdirect
double Equilibrium::getBdirect(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double drdR = 0.0, drdZ = 0.0, dtdR = 0.0, dtdZ = 0.0, jaco2 = 0.0;
    double FF = 0.0, g11 = 0.0, RR = 0.0;

    if (iequmodel == 1)
    {
        // Calculate derivatives and jacobian
        calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);

        // Compute g11
        g11 = std::pow(drdR, 2) + std::pow(drdZ, 2);

        // Get poloidal field and radius
        FF = getfpol_r(rad);
        RR = getR(rad, the);

        // Compute the result based on the formula
        var = std::sqrt((std::pow(getdpsidr(rad), 2) * g11 + std::pow(FF, 2)) /
                        std::pow(RR, 2));
    }
    else if (iequmodel == 2)
    {
        // For model 2, retrieve the magnetic field from getB function
        var = getB(rad, the0);
    }

    return var;
}

// Function to calculate Brad_co_direct
double Equilibrium::getBrad_co_direct(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double drdR = 0.0, drdZ = 0.0, dtdR = 0.0, dtdZ = 0.0, jaco2 = 0.0;
    double g12 = 0.0, RR = 0.0;

    if (iequmodel == 1)
    {
        // Calculate derivatives and jacobian
        calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);

        // Compute g12
        g12 = drdR * dtdR + drdZ * dtdZ;

        // Get radius
        RR = getR(rad, the);

        // Calculate the result using the formula
        var = -jaco2 * g12 * getdpsidr(rad) / RR;
    }
    else if (iequmodel == 2)
    {
        // For model 2, retrieve the Brad_co from getBrad_co function
        var = getBrad_co(rad, the0);
    }

    return var;
}

// Function to calculate Bthe_co_direct
double Equilibrium::getBthe_co_direct(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double drdR = 0.0, drdZ = 0.0, dtdR = 0.0, dtdZ = 0.0, jaco2 = 0.0;
    double g11 = 0.0, RR = 0.0;

    if (iequmodel == 1)
    {
        // Calculate derivatives and jacobian
        calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);

        // Compute g11
        g11 = std::pow(drdR, 2) + std::pow(drdZ, 2);

        // Get radius
        RR = getR(rad, the);

        // Calculate the result based on the formula
        var = jaco2 * g11 * getdpsidr(rad) / RR;
    }
    else if (iequmodel == 2)
    {
        // For model 2, retrieve the Bthe_co from getBthe_co function
        var = getBthe_co(rad, the0);
    }

    return var;
}

// Function to calculate cos_straight_adhoc
double Equilibrium::getcos_straight_adhoc(double rad, double the0,
                                          double rmaxis)
{
    double rormaj = rad / rmaxis;
    double var = -1.0 / rormaj + (1.0 - std::pow(rormaj, 2)) /
                                     (rormaj * (1.0 - rormaj * std::cos(the0)));
    return var;
}

// Function to calculate sin_straight_adhoc
double Equilibrium::getsin_straight_adhoc(double rad, double the0,
                                          double rmaxis)
{
    double rormaj = rad / rmaxis;
    double var = std::sqrt(1.0 - std::pow(rormaj, 2)) * std::sin(the0) /
                 (1.0 - rormaj * std::cos(the0));
    return var;
}

// Function to get R
double Equilibrium::getR(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    int idx = 0, idy = 0, iflag = 0;

    if (iequmodel == 1)
    {
        // Use rrt_bsp to evaluate the radius
        // rrt_bsp->evaluate(rad, the, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getR: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Use rmaxis and getcos_straight_adhoc for model 2
        var = rmaxis + rad * getcos_straight_adhoc(rad, the0, rmaxis);
    }

    return var;
}

// Function to get the derivative of R with respect to rad
double Equilibrium::getdRdrad(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    double rormaj = rad / rmaxis;

    if (iequmodel == 1)
    {
        // Use rrt_bsp to evaluate the derivative with respect to rad
        int idx = 1, idy = 0, iflag = 0;
        // rrt_bsp->evaluate(rad, the, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getdRdrad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Calculate the derivative for model 2
        var = getcos_straight_adhoc(rad, the0, rmaxis) -
              rormaj * std::pow(std::sin(the0), 2) /
                  std::pow(1.0 - rormaj * std::cos(the0), 2);
    }

    return var;
}

// Function to get the derivative of R with respect to the angle (theta)
double Equilibrium::getdRdthe(double rad, double the0)
{
    double var = 0.0;
    double rormaj = rad / rmaxis;

    if (iequmodel == 1)
    {
        // Use rrt_bsp to evaluate the derivative with respect to theta
        int idx = 0, idy = 1, iflag = 0;
        // rrt_bsp->evaluate(rad, the0, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getdRdthe: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Calculate the derivative for model 2
        var = -rad * (1.0 - std::pow(rormaj, 2)) * std::sin(the0) /
              std::pow(1.0 - rormaj * std::cos(the0), 2);
    }

    return var;
}

// Function to get Z
double Equilibrium::getZ(double rad, double the0)
{
    double var = 0.0;
    double the = std::fmod(the0, 2 * M_PI); // Modulo operation
    int idx = 0, idy = 0, iflag = 0;

    if (iequmodel == 1)
    {
        // Use zrt_bsp to evaluate Z
        // zrt_bsp->evaluate(rad, the0, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getZ: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Use zmaxis and getsin_straight_adhoc for model 2
        var = zmaxis + rad * getsin_straight_adhoc(rad, the0, rmaxis);
    }

    return var;
}

// Function to get the derivative of Z with respect to rad
double Equilibrium::getdZdrad(double rad, double the0)
{
    double var = 0.0;
    double rormaj = rad / rmaxis;

    if (iequmodel == 1)
    {
        // Use zrt_bsp to evaluate the derivative with respect to rad
        int idx = 1, idy = 0, iflag = 0;
        // zrt_bsp->evaluate(rad, the0, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getdZdrad: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Calculate the derivative for model 2
        var = getsin_straight_adhoc(rad, the0, rmaxis) +
              rormaj * std::sin(the0) * (std::cos(the0) - rormaj) /
                  (std::sqrt(1.0 - std::pow(rormaj, 2)) *
                   std::pow(1.0 - rormaj * std::cos(the0), 2));
    }

    return var;
}

// Function to get the derivative of Z with respect to theta
double Equilibrium::getdZdthe(double rad, double the0)
{
    double var = 0.0;
    double rormaj = rad / rmaxis;

    if (iequmodel == 1)
    {
        // Use zrt_bsp to evaluate the derivative with respect to theta
        int idx = 0, idy = 1, iflag = 0;
        // zrt_bsp->evaluate(rad, the0, idx, idy, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getdZdthe: " << iflag << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Calculate the derivative for model 2
        var = rad * std::sqrt(1.0 - std::pow(rormaj, 2)) /
              std::pow(1.0 - rormaj * std::cos(the0), 2) *
              (std::cos(the0) - rormaj);
    }

    return var;
}

// Function to compute dqdrloc_rt
double Equilibrium::getdqdrloc_rt(double rad, double the)
{
    double var = 0.0;
    double rormaj = rad / rmaxis;

    if (iequmodel == 1)
    {
        std::cerr << "error: not implemented" << std::endl;
        exit(1); // Exit if model 1 is not implemented
    }
    else if (iequmodel == 2)
    {
        // Calculate based on model 2
        var = 2.0 * c2adhoc * rad + (c1adhoc + c2adhoc * std::pow(rad, 2)) *
                                        rormaj /
                                        ((1.0 - std::pow(rormaj, 2)) * rmaxis);
        var /= std::sqrt(1.0 - std::pow(rormaj, 2));
    }

    return var;
}

// Function to compute qloc_rt
double Equilibrium::getqloc_rt(double rad, double the)
{
    double var = 0.0;
    double drdR, drdZ, dtdR, dtdZ, jaco2, RR;

    if (iequmodel == 1)
    {
        // Call calcdrtdRZ for model 1
        calcdrtdRZ(rad, the, drdR, drdZ, dtdR, dtdZ, jaco2);
        RR = getR(rad, the);
        var = jaco2 * getfpol_r(rad) / (RR * getdpsidr(rad));
    }
    else if (iequmodel == 2)
    {
        // Calculate based on model 2
        var = c1adhoc + c2adhoc * std::pow(rad, 2);
        var /= std::sqrt(1.0 - std::pow(rad / rmaxis, 2));
    }

    return var;
}

// Function to compute psi
double Equilibrium::getpsi(double rad)
{
    double var = 0.0;

    if (iequmodel == 1)
    {
        var = simag + psiwid * std::pow(rad, 2);
    }
    else if (iequmodel == 2)
    {
        var = Bmaxis / (2.0 * c2adhoc) *
              std::log(1.0 + (c2adhoc / c1adhoc) * std::pow(rad, 2));
    }

    return var;
}

// Function to compute dpsidr
double Equilibrium::getdpsidr(double rad)
{
    double var = 0.0;

    if (iequmodel == 1)
    {
        var = 2.0 * psiwid * rad;
    }
    else if (iequmodel == 2)
    {
        var = Bmaxis * rad / (c1adhoc + c2adhoc * std::pow(rad, 2));
    }

    return var;
}
// Function to compute fpol_r
double Equilibrium::getfpol_r(double rad)
{
    double var = 0.0;
    int idx = 0;
    int iflag = 0;

    if (iequmodel == 1)
    {
        // Call evaluate function of F1d_bsp for model 1
        // F1d_bsp.evaluate(getpsi(rad), idx, var, iflag);
        if (iflag != 0)
        {
            std::cerr << "****Error in getfpol_r: " << iflag << ", r=" << rad
                      << ", psi=" << getpsi(rad) << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // Calculate for model 2
        var = Bmaxis * rmaxis;
    }

    return var;
}

// Function to compute dfpoldr_r
double Equilibrium::getdfpoldr_r(double rad)
{
    double var = 0.0;
    int idx = 1;
    int iflag = 0;

    if (iequmodel == 1)
    {
        // Call evaluate function of F1d_bsp for model 1
        // F1d_bsp.evaluate(getpsi(rad), idx, var, iflag);
        var = var * 2.0 * psiwid * rad;
        if (iflag != 0)
        {
            std::cerr << "****Error in getdfpoldr_r: " << iflag << ", r=" << rad
                      << ", psi=" << getpsi(rad) << std::endl;
        }
    }
    else if (iequmodel == 2)
    {
        // For model 2, set var to 0
        var = 0.0;
    }

    return var;
}

// Function to compute jaco2
double Equilibrium::getjaco2(double rad, double the)
{
    double var = 0.0;
    double dRdrad = getdRdrad(rad, the);
    double dRdthe = getdRdthe(rad, the);
    double dZdrad = getdZdrad(rad, the);
    double dZdthe = getdZdthe(rad, the);

    var = dRdrad * dZdthe - dRdthe * dZdrad;

    return var;
}

// Function to compute jaco3
double Equilibrium::getjaco3(double rad, double the)
{
    double var = 0.0;
    double dRdrad = getdRdrad(rad, the);
    double dRdthe = getdRdthe(rad, the);
    double dZdrad = getdZdrad(rad, the);
    double dZdthe = getdZdthe(rad, the);
    double RR = getR(rad, the);

    var = RR * (dRdrad * dZdthe - dRdthe * dZdrad);

    return var;
}